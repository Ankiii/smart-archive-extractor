...
#$validExtensions = @('.zip', '.7z', '.gz', '.tgz', '.tar', '.rar')
...
$LogFile = Join-Path $TargetDirectory "extraction_log_$logTimestamp.txt"
...
do {
    ...
    foreach ($archive in $archives) {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            # --- Handle .tgz / .tar.gz double-extraction quirk ---
            if ($archive.Name.ToLower() -match '\.tar\.gz$' -or $archive.Extension.ToLower() -eq '.tgz') {
                $expectedTarName = $archive.BaseName
